...
const MUSIC_VIDEO_PROJECTS: Project[] = [
  {
    id: "1",
...
const N = MUSIC_VIDEO_PROJECTS.length;
const EXTENDED_PROJECTS: Project[] = [MUSIC_VIDEO_PROJECTS[N - 1], ...MUSIC_VIDEO_PROJECTS, MUSIC_VIDEO_PROJECTS[0]];
const SIDEBAR_COPIES = 7;
const INFINITE_SIDEBAR_PROJECTS: Project[] = Array.from({ length: SIDEBAR_COPIES }, () => MUSIC_VIDEO_PROJECTS).flat();
...
export default function MusicVideoPage() {
  ...
  const activeProject = MUSIC_VIDEO_PROJECTS[selectedIndex];
            ...
            {EXTENDED_PROJECTS.map((project, idx) => {
              ...
              return (
                <div key={`main-track-${project.id}-${idx}`} className="w-full h-screen flex-shrink-0 flex items-center justify-center" style={{ padding: "24px 40px" }}>
                  <Link href={`/work/music-video/${project.id}`} className="relative w-full aspect-[16/10] pointer-events-auto group block cursor-pointer">
